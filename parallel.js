...
function data_table(sample) {
  ...
  table
    .append("span")
      ...
      .style("background", function(d) {return color(d.playlist_genre,0.85) })
...
function highlight(d) {
  ...
  d3.selectAll(".row").style("opacity", function(p) { return (d.playlist_genre == p) ? null : "0.3" });
...
function brush() {
  ...
  var tallies = _(selected)
    .groupBy(function(d) { return d.playlist_genre; })
...
function search(selection,str) {
  ...
  return _(selection).filter(function(d) { return pattern.exec(d.track_name); });
}
